...
public class SeekBarPreference extends DialogPreference {
    ...
    public SeekBarPreference(Context context, AttributeSet attrs) { super(context, attrs);init(context, attrs); }
    ...
    @Override protected Float onGetDefaultValue(TypedArray a, int index) { return a.getFloat(index, 0.0f); }
    @Override protected void onSetInitialValue(boolean restorePersistedValue, Object defaultValue) {
        if (restorePersistedValue) setVal(getPersistedFloat(0.0f)); else setVal((Float) defaultValue);
        savePrevVal();
    }
    private String formatFloatDisplay(Float val) {
        if (mAsPercent) return String.format("%d%%", (int) (val * 100));
        if (mDisplayFormat != null) return String.format(mDisplayFormat, val); else return Float.toString(val);
    }
    ...
    private int getPercent(float val, float min, float max) { return (int) (100 * (val - min) / (max - min)); }
    private int getProgressVal() {
        if (mLogScale) return getPercent((float) Math.log(mVal), (float) Math.log(mMin), (float) Math.log(mMax));
        else return getPercent(mVal, mMin, mMax);
    }
        ...
        mSeek.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if (fromUser) {
                    ...
                    if (newVal != mVal) onChange(newVal);
                    setVal(newVal);
                    mSeek.setProgress(getProgressVal());
                }
                showVal();
    ...
    public void onChange(float val) { /* override in subclasses*/}
    @Override public CharSequence getSummary() { return formatFloatDisplay(mVal); }
    @Override protected void onDialogClosed(boolean positiveResult) {
        if (!positiveResult) { restoreVal();return; }
        if (shouldPersist()) { persistFloat(mVal);savePrevVal(); }
        notifyChanged();
    }
}
